...
namespace UnityStandardAssets.Vehicles.Car{
    ...
    public class CarController : MonoBehaviour  {
        ...
        private void ApplyDrive(float accel, float footbrake){
            float thrustTorque = CarDriveType.FourWheelDrive == m_CarDriveType ? accel * (m_CurrentTorque / 4f) : accel * (m_CurrentTorque / 2f);
            if(m_CarDriveType == CarDriveType.FourWheelDrive) for(int i = 0; i < 4; i++) m_WheelColliders[i].motorTorque = thrustTorque;
            else if(m_CarDriveType == CarDriveType.FrontWheelDrive) m_WheelColliders[0].motorTorque = m_WheelColliders[1].motorTorque = thrustTorque;
            else m_WheelColliders[2].motorTorque = m_WheelColliders[3].motorTorque = thrustTorque;
            for(int i = 0; i < 4; i++){
        ...
        private void SteerHelper() {
            for(int i = 0; i < 4; i++) {
                ...
                if (wheelhit.normal == Vector3.zero) return; // wheels arent on the ground so dont realign the rigidbody velocity
            }
        ...
        private void AddDownForce(){m_WheelColliders[0].attachedRigidbody.AddForce(-transform.up * m_Downforce * m_WheelColliders[0].attachedRigidbody.velocity.magnitude);}
        ...
        private void AdjustTorque(float forwardSlip){
            if(forwardSlip >= m_SlipLimit && m_CurrentTorque >= 0) m_CurrentTorque -= 10 * m_TractionControl;
            else{
